...
void loop() {
  ...
  if ( network.available() ) {     // Is there any incoming data?
    ...
    if (header.from_node == 1) {   // If data comes from Node 02
      ...
      if (incomingData == 0 or incomingData == 1) {
        d = incomingData;
        ...
        if (incomingData == 7 or incomingData == 8) {
  ...
  if (digitalRead(button2) == 0) {
    unsigned long buttonState2 = digitalRead(button2) + 7;
    RF24NetworkHeader header2(node01);     // (Address where the data is going)
    bool ok = network.write(header2, &buttonState2, sizeof(buttonState2)); // Send the data
